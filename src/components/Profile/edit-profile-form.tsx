...
import { createListCollection, type ListCollection, Stack } from '@chakra-ui/react';
...
export function PersonalDataFormManager({ isAdminEdit, memberId, memberPromise }: PersonalDataFormProps) {
    const member = use(memberPromise);

    const [memberTypes] = useState<ListCollection<ICollection>>(() => {
        return createListCollection({
            items: MEMBER_TYPES_WITH_NAMES.filter((type) => Number(type.value) !== MemberTypes.Member),
        });
    });
    ...
    const { register, handleSubmit, formState, reset } = useForm<PersonalDataFormType>({
    ...
    return (
        <Stack as="form" onSubmit={handleSubmit(handleUpdate)} gap={4} py={4} mb={[36, 10]}>
            <Input
                type="text"
                label="Nome completo"
            ...
            {isAdminEdit && (
                <Select
                    ...
                    isCard={false}
                    disabled={member.memberType === MemberTypes.Member}
                />
            )}
            <Input type="email" disabled defaultValue={member.email} label="E-mail" isCard={false} />
            <Button
                type="submit"
                colorPalette="brand"
                alignSelf="flex-end"
